...
- `.claude/skills/add-provider/SKILL.md`, the instructions that turn a vendor's
  documentation into a manifest. It is a document rather than a program because
  the three things worth having, the paginator, the projection and what a bare
  403 means, are the three an API spec does not carry (#68).
- `stripe`, read only, at `experimental`. Four tools, generated by following
  that skill and never run against the vendor, which is what `experimental`
  means here (#68). Its list tools take the filters the vendor
  documents: an email on customers, a customer, payment intent and transfer
  group on charges, and a created range on both (#73).

### Fixed

- An id-paged provider can page past the first page when its ids are not
  numbers. The executor minted `cus_24` as a cursor and then refused it,
  because only the `cursor` style was treated as upstream-minted. Discord
  survived because a snowflake parses as an integer (#71).
